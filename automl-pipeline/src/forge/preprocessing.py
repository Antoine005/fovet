"""
Feature normalization — StandardScaler wrapper for Ardent Cast pipelines.
...
class Scaler:
    ...
    def export_c_header(self, output_dir: Path, stem: str) -> Path:
        """Write ard_scaler_params.h with static arrays for firmware normalization.

        The generated header is ready to include on the ESP32:

            #include "ard_scaler_params.h"
            // Apply: normalized[i] = (raw[i] - ard_scaler_mean[i]) / ard_scaler_scale[i]
        """
        if self.mean_ is None:
            raise RuntimeError("Scaler must be fitted before export.")
        output_dir.mkdir(parents=True, exist_ok=True)
        ...
        header = (
            ...
            " */\n"
            "#ifndef ARD_SCALER_PARAMS_H\n"
            "#define ARD_SCALER_PARAMS_H\n"
            "\n"
            f"#define ARD_SCALER_N_FEATURES {n}\n"
            "\n"
            "/* Per-feature mean (subtract from raw reading) */\n"
            f"static const float ard_scaler_mean[{n}]  = {{ {mean_vals} }};\n"
            "\n"
            "/* Per-feature scale (divide after subtracting mean) */\n"
            f"static const float ard_scaler_scale[{n}] = {{ {scale_vals} }};\n"
            "\n"
            "#endif /* ARD_SCALER_PARAMS_H */\n"
        )

        out_path = output_dir / "ard_scaler_params.h"
