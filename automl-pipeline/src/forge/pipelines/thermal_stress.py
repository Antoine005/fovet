...
Export (in output_dir/):
  ...
  thermal_thresholds.h           — C header for Ardent Pulse MCU (H3.3)
...
DataFrame format (Ardent standard):
...
def extract_features(
    ...
    *,
    ...
    """Slide a window over a Ardent TEMP DataFrame and extract thermal features.

    Only rows with sensor_type == "temp" are used (if the column is present).

    Args:
        df:             Ardent-format DataFrame (sensor_type, value_1=celsius,
...
class ThermalStressPipeline:
    ...
    def fit(
        ...
        *,
        ...
        Args:
            df:        Ardent-format DataFrame (sensor_type="temp", value_1=celsius,
                       value_2=humidity).
            label_col: Binary label column (1=stress, 0=normal).
    ...
    def evaluate(
        ...
        *,
        ...
        Args:
            df:        Ardent-format DataFrame with celsius, humidity and labels.
    ...
    def export(self, output_dir: Path) -> dict[str, Path]:
        ...
        # --- C header for Ardent Pulse MCU (H3.3) ---
        ...
        h_lines = [
            ...
            "#ifndef ARD_THERMAL_THRESHOLDS_H",
            "#define ARD_THERMAL_THRESHOLDS_H",
            "",
            "/**",
            ...
            " */",
            "",
            f"#define ARD_TEMP_N_FEATURES          {N_FEATURES}",
            f"#define ARD_TEMP_WINDOW_S             {int(self.window_s)}",
            f"#define ARD_TEMP_STEP_S               {int(self.step_s)}",
            "",
            "/* WBGT thresholds (°C) — ISO 7243, moderate physical work     */",
            f"#define ARD_TEMP_WBGT_WARN_C          {WBGT_WARN_C:.1f}f",
            f"#define ARD_TEMP_WBGT_DANGER_C        {WBGT_DANGER_C:.1f}f",
            "",
            "/* Cold stress threshold (°C) — hypothermia risk               */",
            f"#define ARD_TEMP_COLD_ALERT_C         {COLD_ALERT_C:.1f}f",
            "",
            "/* Normal condition statistics (from training scaler)           */",
            f"#define ARD_TEMP_NORMAL_CELSIUS_MEAN  {normal_c_mean:.2f}f",
            f"#define ARD_TEMP_NORMAL_CELSIUS_STD   {normal_c_std:.2f}f",
            f"#define ARD_TEMP_NORMAL_HUMIDITY_MEAN {normal_h_mean:.2f}f",
            "",
            "#endif /* ARD_THERMAL_THRESHOLDS_H */",
        ]
