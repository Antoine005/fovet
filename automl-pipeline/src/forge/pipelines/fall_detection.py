...
class FallDetectionPipeline:
    ...
    def fit(
        ...
        *,
        ...
        self._model = keras.Model(inputs, output, name="ard_fall_detection")
    ...
    def export(self, output_dir: Path) -> dict[str, Path]:
        ...
        # .h
        guard = "ARD_FALL_DETECTION_MODEL_H"
        h_lines = [
            ...
            "#include <stdint.h>",
            "",
            f"#define ARD_FALL_DETECTION_N_FEATURES  {N_FEATURES}",
            f"#define ARD_FALL_DETECTION_THRESHOLD   {self.threshold:.6f}f",
            f"#define ARD_FALL_DETECTION_WINDOW      {self.window_samples}",
            "",
