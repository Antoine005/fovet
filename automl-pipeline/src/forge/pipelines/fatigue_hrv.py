...
with any BVP/PPG signal that follows the standard Ardent CSV format:
...
Export (in output_dir/):
  ...
  fatigue_hrv_thresholds.h       — C header for Ardent Pulse MCU (H2.3)
...
def extract_features(
    ...
    *,
    ...
    """Slide a window over a Ardent HR DataFrame and extract HRV features.
    ...
    Args:
        df:             Ardent-format DataFrame (sensor_type, value_1 = BVP).
...
class FatigueHRVPipeline:
    ...
    def fit(
        ...
        *,
        ...
        Args:
            df:        Ardent-format DataFrame (sensor_type="hr", value_1=BVP).
    ...
    def predict_proba(self, df: pd.DataFrame) -> np.ndarray:
        ...
        Args:
            df: Ardent-format DataFrame with BVP signal.
    ...
    def evaluate(
        ...
        *,
        ...
        Args:
            df:        Ardent-format DataFrame with BVP and labels.
    ...
    def export(self, output_dir: Path) -> dict[str, Path]:
        ...
        Produces in output_dir/:
          - fatigue_hrv_model.pkl        (serialized RandomForest — joblib)
          - fatigue_hrv_config.json      (metadata: features, scaler, threshold)
          - fatigue_hrv_thresholds.h     (C header for Ardent Pulse MCU)
        ...
        # --- C header for Ardent Pulse MCU (H2.3) ---
        ...
        h_lines = [
            ...
            "#ifndef ARD_FATIGUE_HRV_THRESHOLDS_H",
            "#define ARD_FATIGUE_HRV_THRESHOLDS_H",
            "",
            "/**",
            ...
            " */",
            "",
            f"#define ARD_FATIGUE_N_FEATURES       {N_FEATURES}",
            f"#define ARD_FATIGUE_SAMPLE_RATE_HZ   {self.sample_rate_hz}",
            f"#define ARD_FATIGUE_WINDOW_S          {self.window_s}",
            f"#define ARD_FATIGUE_STEP_S            {self.step_s}",
            "",
            "/* RMSSD thresholds (ms) — primary fatigue indicator           */",
            "/* RMSSD > ARD_FATIGUE_RMSSD_OK    : no fatigue             */",
            "/* RMSSD < ARD_FATIGUE_RMSSD_ALERT : critical fatigue       */",
            "/* in between                         : alert state            */",
            f"#define ARD_FATIGUE_RMSSD_OK          {rmssd_ok:.1f}f",
            f"#define ARD_FATIGUE_RMSSD_ALERT       {rmssd_alert:.1f}f",
            "",
            "/* Mean HR thresholds (bpm) — secondary indicator             */",
            "/* HR < ARD_FATIGUE_HR_OK           : relaxed               */",
            "/* HR > ARD_FATIGUE_HR_ALERT        : elevated, possible fatigue */",
            f"#define ARD_FATIGUE_HR_OK             {hr_ok:.1f}f",
            f"#define ARD_FATIGUE_HR_ALERT          {hr_alert:.1f}f",
            "",
            "#endif /* ARD_FATIGUE_HRV_THRESHOLDS_H */",
        ]
