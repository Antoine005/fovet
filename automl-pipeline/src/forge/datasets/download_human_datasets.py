...
-------------------------------------------------------------------------
download_human_datasets.py — Biosignal dataset manager for Ardent Forge
...
All datasets are parsed into a standard Ardent CSV format:
...
def quality_report(df: pd.DataFrame, dataset_name: str = "unknown") -> DatasetQualityReport:
    """Compute quality report from a parsed Ardent-format DataFrame."""
    warnings: list[str] = []
...
def _make_standard_df(
    ...
    """Build a standard Ardent CSV DataFrame."""
    n = len(timestamps_ms)
    if values.ndim == 1:
...
def parse_upfall(raw_dir: Path) -> pd.DataFrame:
    ...
    Returns a standard Ardent CSV DataFrame.
...
def parse_wesad(raw_dir: Path) -> pd.DataFrame:
    ...
    Returns a standard Ardent CSV DataFrame using wrist ACC (sensor_type='imu')
...
def load_parsed(output_dir: Path, dataset_name: str) -> pd.DataFrame:
    """Load a previously parsed Ardent CSV.
    ...
    Returns:
        DataFrame in standard Ardent format.
...
def _write_readme(output_dir: Path, dataset_name: str, report: DatasetQualityReport) -> None:
    ...
    lines = [
        f"# Ardent Forge — {dataset_name} (parsed)",
        "",
    ...
    lines += [
        "",
        "---",
        "*Auto-generated by Ardent Forge download_human_datasets.py*",
    ]
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        ...
        description="Ardent Forge — Human biosignal dataset manager",
    )
    ...
    parse_p = sub.add_parser("parse", help="Parse raw dataset files into standard Ardent CSV")
