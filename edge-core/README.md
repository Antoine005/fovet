...
**Demo complète (Z-Score + Drift + MQTT → Watch)** :
...
│   ├── test_forge_integration.c ← 12 tests : validation header Forge→Pulse
...
typedef struct {
    float    window[ARD_MAD_MAX_WINDOW]; // ring buffer des derniers samples (128 max)
    float    scratch[ARD_MAD_MAX_WINDOW];// zone de tri temporaire — ne pas toucher
...
// sizeof(ArdentMAD) ≈ 1040 bytes pour win_size=128 (configurable via ARD_MAD_MAX_WINDOW)
```
...
ArdentMAD detector;
ard_mad_init(&detector, 32, 3.5f);  // fenêtre 32 samples, seuil 3.5 MAD

while (1) {
    float sample = read_sensor();
    if (ard_mad_update(&detector, sample)) {
        trigger_alert();
    }
}
```

### Réduire la RAM — `ARD_MAD_MAX_WINDOW`

La taille du ring buffer est fixée à la compilation par `ARD_MAD_MAX_WINDOW` (défaut : 128).
Pour économiser de la RAM sur un microcontrôleur avec peu de mémoire :

```c
// Avant d'inclure le header, réduire la taille max :
#define ARD_MAD_MAX_WINDOW 32
#include "ardent/mad.h"
// ArdentMAD occupe désormais ~264 bytes au lieu de ~1040 bytes
```

### Démarrage précalibré (Forge → Pulse)

Ardent Forge calibre le seuil hors-ligne et exporte un header prêt à l'emploi :

```c
// Généré par : uv run forge run --config configs/mon_capteur.yaml
// Fichier    : models/ard_mad_config.h

#include "ardent/mad.h"

static ArdentMAD ard_mad_value = {
...
```c
// Généré par : uv run forge run --config configs/mon_capteur.yaml
// Fichier    : models/ard_zscore_config.h
...
3. Copier models/ard_zscore_config.h dans src/
4. #include "ard_zscore_config.h"  ← remplace ard_zscore_init()
```
...
Tout passe par des fonctions `hal_*` définies dans `include/ardent/hal/`.
...
**Ajouter un nouveau driver :** créer `include/ardent/drivers/<capteur>.h` +
