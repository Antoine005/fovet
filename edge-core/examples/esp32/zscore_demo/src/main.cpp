...
/* Conditionally include Forge-calibrated detector config.
 * Generated by `forge run` — copy to src/ or deploy via Ardent Watch.
 * When present: detector uses pre-calibrated mean/stddev from training data.
 * When absent:  detector starts with hardcoded defaults (demo mode). */
#if __has_include("ard_zscore_config.h")
#  include "ard_zscore_config.h"    /* provides ARD_FORGE_CALIBRATED + ARD_FORGE_ZSCORE_PRIMARY */
#endif
#if __has_include("ard_drift_config.h")
#  include "ard_drift_config.h"     /* provides ARD_FORGE_DRIFT_CALIBRATED + ARD_FORGE_DRIFT_PRIMARY */
#endif
...
void setup(void)
{
    ...
    /* Initialise detectors — use Forge-calibrated params if available */
#ifdef ARD_FORGE_CALIBRATED
    g_zscore = ARD_FORGE_ZSCORE_PRIMARY;        /* pre-calibrated: mean/stddev from training data */
    hal_uart_print("[Forge] Z-Score pre-calibrated from Forge model\r\n");
#else
    ard_zscore_init(&g_zscore, ZSCORE_THRESHOLD, ZSCORE_MIN_SAMPLES);
#endif

#ifdef ARD_FORGE_DRIFT_CALIBRATED
    g_drift = ARD_FORGE_DRIFT_PRIMARY;          /* pre-calibrated threshold from training data */
    hal_uart_print("[Forge] Drift pre-calibrated from Forge model\r\n");
#else
