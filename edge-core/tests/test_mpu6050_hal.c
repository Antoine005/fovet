...
#include "../include/ardent/hal/mpu6050_hal.h"
...
static void setup(void)
{
    ...
    ard_mpu6050_reset();
    ard_mpu6050_set_i2c(mock_i2c_write, mock_i2c_read);
...
static void test_init_valid_chip_id_68(void)
{
    setup();
    s_regs[REG_WHO_AM_I] = 0x68;
    ASSERT_INT_EQ(ard_hal_imu_init(0x68), ARD_HAL_OK,
                  "init: chip ID 0x68 returns ARD_HAL_OK");
}
...
static void test_init_valid_chip_id_69(void)
{
    ...
    ASSERT_INT_EQ(ard_hal_imu_init(0x69), ARD_HAL_OK,
                  "init: chip ID 0x69 returns ARD_HAL_OK");
}
...
static void test_init_invalid_chip_id(void)
{
    ...
    ASSERT_INT_EQ(ard_hal_imu_init(0x68), ARD_MPU_ERR_ID,
                  "init: wrong chip ID returns ARD_MPU_ERR_ID");
}
...
static void test_init_i2c_read_error(void)
{
    setup();
    s_i2c_fail_read = 1;
    ASSERT_INT_EQ(ard_hal_imu_init(0x68), ARD_MPU_ERR_I2C,
                  "init: I2C read error returns ARD_MPU_ERR_I2C");
}
...
static void test_init_i2c_write_error(void)
{
    ...
    s_i2c_fail_write = 1;
    ASSERT_INT_EQ(ard_hal_imu_init(0x68), ARD_MPU_ERR_I2C,
                  "init: I2C write error returns ARD_MPU_ERR_I2C");
}
...
static void test_init_auto_registers_with_hal(void)
{
    ...
    ASSERT_INT_EQ(ard_hal_biosignal_read(ARD_SOURCE_IMU, &s), ARD_HAL_OK,
...
static void test_read_returns_ok(void)
{
    ...
    ard_hal_imu_init(0x68);
    ard_biosignal_sample_t s;
    ASSERT_INT_EQ(ard_hal_imu_read(&s), ARD_HAL_OK,
                  "read: returns ARD_HAL_OK");
}
...
static void test_read_source_is_imu(void)
{
    ...
    ard_biosignal_sample_t s;
    ard_hal_imu_read(&s);
    ASSERT(s.source == ARD_SOURCE_IMU, "read: source == ARD_SOURCE_IMU");
}
...
static void test_read_null_returns_err_null(void)
{
    setup();
    ard_hal_imu_init(0x68);
    ASSERT_INT_EQ(ard_hal_imu_read(NULL), ARD_HAL_ERR_NULL,
                  "read: NULL out returns ARD_HAL_ERR_NULL");
}
...
static void test_read_before_init_returns_error(void)
{
    ...
    ard_biosignal_sample_t s;
    ASSERT_INT_EQ(ard_hal_imu_read(&s), ARD_MPU_ERR_I2C,
                  "read: before init returns ARD_MPU_ERR_I2C");
}
...
static void test_read_timestamp_set(void)
{
    ...
    ASSERT(s.source == ARD_SOURCE_IMU, "read: timestamp_ms test — source still correct after read");
}
...
static void test_read_i2c_error(void)
{
    ...
    ard_biosignal_sample_t s;
    ASSERT_INT_EQ(ard_hal_imu_read(&s), ARD_MPU_ERR_I2C,
                  "read: I2C error returns ARD_MPU_ERR_I2C");
}
...
{
    ard_biosignal_sample_t s;
    s.source = ARD_SOURCE_IMU;
    s.value.imu.ax = 0.0f;
    s.value.imu.ay = 0.0f;
    s.value.imu.az = 1.0f; /* standing still on earth */
    ASSERT_FLOAT_EQ(ard_hal_imu_get_magnitude(&s), 1.0f, 1e-5f,
...
static void test_magnitude_pythagorean(void)
{
    ...
    s.value.imu.ay = 4.0f;
    s.value.imu.az = 0.0f;
    ASSERT_FLOAT_EQ(ard_hal_imu_get_magnitude(&s), 5.0f, 1e-4f,
                    "magnitude: (3,4,0)g = 5.0g (3-4-5 triplet)");
}

static void test_magnitude_null_ptr(void)
{
    ASSERT_FLOAT_EQ(ard_hal_imu_get_magnitude(NULL), 0.0f, 1e-9f,
...
static void test_sample_rate_25hz(void)
{
    setup();
    ard_hal_imu_init(0x68);
    ard_hal_imu_set_sample_rate(25);
...
static void test_sample_rate_200hz(void)
{
    setup();
    ard_hal_imu_init(0x68);
    ard_hal_imu_set_sample_rate(200);
...
static void test_sample_rate_clamped_high(void)
{
    setup();
    ard_hal_imu_init(0x68);
    ard_hal_imu_set_sample_rate(9999); /* beyond max → clamped to 200 */
    ASSERT_INT_EQ((int)s_regs[REG_SMPLRT_DIV], 4,
...
static void test_sample_rate_clamped_low(void)
{
    setup();
    ard_hal_imu_init(0x68);
    ard_hal_imu_set_sample_rate(1); /* below min → clamped to 10 */
