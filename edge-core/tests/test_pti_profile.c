...
/*
 * Native unit tests for fovet_pti_profile — compile with gcc, no hardware needed.
 *
...
#include "../include/fovet/profiles/pti_profile.h"
#include "../include/fovet/hal/fovet_biosignal_hal.h"
#include "../include/fovet/hal/mpu6050_hal.h"  /* for FOVET_MPU_ERR_I2C */
...
static int mock_imu_read(fovet_biosignal_sample_t *out)
{
    if (s_imu_fail)
...
/* Alert capture */
static fovet_pti_alert_t s_last_alert;
static int               s_alert_count   = 0;
static int               s_fall_count    = 0;
static int               s_motionless_count = 0;
static int               s_sos_count     = 0;

static void capture_alert(fovet_pti_alert_t alert, void *user_data)
{
    (void)user_data;
    s_last_alert = alert;
    s_alert_count++;
    if (alert == FOVET_ALERT_FALL)       s_fall_count++;
    if (alert == FOVET_ALERT_MOTIONLESS) s_motionless_count++;
    if (alert == FOVET_ALERT_SOS)        s_sos_count++;
}
...
static fovet_pti_ctx_t   g_ctx;
static fovet_pti_config_t g_cfg;
...
static void setup(void)
{
    ...
    fovet_hal_biosignal_register(FOVET_SOURCE_IMU, mock_imu_read);

    g_cfg = fovet_pti_default_config();
    fovet_pti_init(&g_ctx, &g_cfg,
...
static void run_ticks(fovet_pti_ctx_t *ctx, uint32_t n, uint32_t dt_ms)
{
    uint32_t i;
    for (i = 0; i < n; i++)
    {
...
{
    fovet_pti_ctx_t ctx;
    fovet_pti_init(&ctx, NULL, capture_alert, mock_fall_score, mock_gpio_read, NULL, NULL);
...
{
    /* Must not crash */
    fovet_pti_init(NULL, NULL, capture_alert, mock_fall_score, mock_gpio_read, NULL, NULL);
...
{
    setup();
    ASSERT_INT_EQ(fovet_pti_tick(&g_ctx), FOVET_HAL_OK,
                  "tick returns FOVET_HAL_OK on success");
}
...
static void test_tick_imu_failure_returns_error(void)
{
    setup();
    s_imu_fail = 1;
    ASSERT_INT_EQ(fovet_pti_tick(&g_ctx), FOVET_MPU_ERR_I2C,
...
static void test_no_fall_alert_before_window_full(void)
{
    ...
    /* Run fewer ticks than FOVET_PTI_WINDOW_SIZE - 1 */
    run_ticks(&g_ctx, FOVET_PTI_WINDOW_SIZE - 1, 40);
...
static void test_fall_alert_when_score_high(void)
{
    ...
    s_mock_score = 0.0f;
    run_ticks(&g_ctx, FOVET_PTI_WINDOW_SIZE, 40);

    /* Now raise score above threshold */
    s_mock_score = 0.90f;
    fovet_pti_tick(&g_ctx);
...
static void test_no_fall_alert_when_score_below_threshold(void)
{
    ...
    run_ticks(&g_ctx, FOVET_PTI_WINDOW_SIZE + 5, 40);
    ASSERT_INT_EQ(s_fall_count, 0,
...
static void test_fall_alert_fires_every_tick_while_score_high(void)
{
    setup();
    s_mock_score = 0.95f;
    /* Window fills on tick FOVET_PTI_WINDOW_SIZE, then each subsequent tick fires */
    run_ticks(&g_ctx, FOVET_PTI_WINDOW_SIZE + 3, 40);
...
static void test_motionless_alert_after_timeout(void)
{
    ...
    fovet_pti_init(&g_ctx, &g_cfg, capture_alert, mock_fall_score, mock_gpio_read, NULL, NULL);

    /* Magnitude = sqrt(0² + 0² + 0.1²) = 0.1 < 0.5 → motionless */
    s_imu_ax = 0.0f; s_imu_ay = 0.0f; s_imu_az = 0.1f;

    /* Advance time beyond timeout */
    fovet_pti_tick(&g_ctx);            /* t=0 → last_motion set at init */
    s_mock_time_ms = 1001U;
    fovet_pti_tick(&g_ctx);            /* elapsed > 1000 → alert */
...
static void test_motionless_alert_sent_only_once(void)
{
    ...
    fovet_pti_tick(&g_ctx);
    s_mock_time_ms = 200U;
    fovet_pti_tick(&g_ctx);  /* fires alert */
    s_mock_time_ms = 300U;
    fovet_pti_tick(&g_ctx);  /* debounce — should NOT fire again */
...
static void test_motionless_clears_on_motion(void)
{
    ...
    fovet_pti_tick(&g_ctx);
    s_mock_time_ms = 200U;
    fovet_pti_tick(&g_ctx);  /* fires MOTIONLESS */
    ...
    fovet_pti_tick(&g_ctx);  /* clears motionless_alert_sent */

    /* Phase 3: motionless again after new timeout */
    s_imu_ax = 0.0f; s_imu_ay = 0.0f; s_imu_az = 0.1f;
    fovet_pti_tick(&g_ctx);
    s_mock_time_ms += 200U;
    fovet_pti_tick(&g_ctx);  /* should fire again */
...
static void test_sos_re_fires_after_release(void)
{
    setup();
    s_mock_gpio = 0;
    fovet_pti_tick(&g_ctx);   /* fires */
    s_mock_gpio = 1;
    fovet_pti_tick(&g_ctx);   /* release */
    s_mock_gpio = 0;
    fovet_pti_tick(&g_ctx);   /* press again → fires */
...
static void test_reset_alerts_clears_motionless_sent(void)
{
    ...
    fovet_pti_tick(&g_ctx);
    s_mock_time_ms = 200U;
    fovet_pti_tick(&g_ctx);  /* fires MOTIONLESS, sets debounce */

    fovet_pti_reset_alerts(&g_ctx);  /* clears debounce */
...
static void test_sleep_fn_called_per_tick(void)
{
    setup();
    s_slept_ms = 0;
    fovet_pti_init(&g_ctx, &g_cfg, capture_alert, mock_fall_score, mock_gpio_read,
                   mock_sleep, NULL);
    run_ticks(&g_ctx, 3, 0);
