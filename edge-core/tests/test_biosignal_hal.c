...
static int stub_imu_read(fovet_biosignal_sample_t *out)
{
...
static int stub_hr_read(fovet_biosignal_sample_t *out)
{
...
static int stub_temp_read(fovet_biosignal_sample_t *out)
{
...
static int stub_ecg_read(fovet_biosignal_sample_t *out)
{
    out->source            = FOVET_SOURCE_ECG;
...
static int stub_error_read(fovet_biosignal_sample_t *out)
{
    (void)out;
...
static void test_register_returns_ok(void)
{
    ...
    ASSERT_INT_EQ(fovet_hal_biosignal_register(FOVET_SOURCE_IMU, stub_imu_read),
                  FOVET_HAL_OK,
                  "register IMU returns FOVET_HAL_OK");
}

static void test_register_null_fn_returns_err_null(void)
{
    fovet_hal_biosignal_reset();
    ASSERT_INT_EQ(fovet_hal_biosignal_register(FOVET_SOURCE_IMU, NULL),
                  FOVET_HAL_ERR_NULL,
                  "register NULL fn returns FOVET_HAL_ERR_NULL");
}
...
static void test_register_out_of_range_returns_err_type(void)
{
    fovet_hal_biosignal_reset();
    ASSERT_INT_EQ(
        fovet_hal_biosignal_register(
            (fovet_biosignal_source_t)FOVET_BIOSIGNAL_SOURCE_COUNT,
            stub_imu_read),
        FOVET_HAL_ERR_TYPE,
        "register out-of-range type returns FOVET_HAL_ERR_TYPE");
}

static void test_read_null_out_returns_err_null(void)
{
    fovet_hal_biosignal_reset();
    fovet_hal_biosignal_register(FOVET_SOURCE_IMU, stub_imu_read);
    ASSERT_INT_EQ(fovet_hal_biosignal_read(FOVET_SOURCE_IMU, NULL),
                  FOVET_HAL_ERR_NULL,
                  "read with NULL out returns FOVET_HAL_ERR_NULL");
}
...
static void test_read_out_of_range_returns_err_type(void)
{
    ...
    ASSERT_INT_EQ(
        fovet_hal_biosignal_read(
            (fovet_biosignal_source_t)FOVET_BIOSIGNAL_SOURCE_COUNT, &s),
        FOVET_HAL_ERR_TYPE,
        "read out-of-range type returns FOVET_HAL_ERR_TYPE");
}
...
static void test_read_no_handler_returns_err_noreg(void)
{
    fovet_hal_biosignal_reset();
    fovet_biosignal_sample_t s;
    ASSERT_INT_EQ(fovet_hal_biosignal_read(FOVET_SOURCE_HR, &s),
                  FOVET_HAL_ERR_NOREG,
                  "read unregistered source returns FOVET_HAL_ERR_NOREG");
}
...
static void test_read_imu_ok(void)
{
    ...
    fovet_hal_biosignal_register(FOVET_SOURCE_IMU, stub_imu_read);
    fovet_biosignal_sample_t s;
    ASSERT_INT_EQ(fovet_hal_biosignal_read(FOVET_SOURCE_IMU, &s),
                  FOVET_HAL_OK,
                  "read IMU returns FOVET_HAL_OK");
}
...
static void test_read_imu_payload(void)
{
    ...
    ASSERT(s.source == FOVET_SOURCE_IMU,       "IMU sample.source == FOVET_SOURCE_IMU");
...
{
    fovet_hal_biosignal_reset();
    fovet_hal_biosignal_register(FOVET_SOURCE_HR, stub_hr_read);
    fovet_biosignal_sample_t s;
    fovet_hal_biosignal_read(FOVET_SOURCE_HR, &s);

    ASSERT(s.source == FOVET_SOURCE_HR,        "HR sample.source == FOVET_SOURCE_HR");
...
{
    fovet_hal_biosignal_reset();
    fovet_hal_biosignal_register(FOVET_SOURCE_TEMP, stub_temp_read);
    fovet_biosignal_sample_t s;
    fovet_hal_biosignal_read(FOVET_SOURCE_TEMP, &s);

    ASSERT(s.source == FOVET_SOURCE_TEMP,      "TEMP sample.source == FOVET_SOURCE_TEMP");
...
{
    fovet_hal_biosignal_reset();
    fovet_hal_biosignal_register(FOVET_SOURCE_ECG, stub_ecg_read);
    fovet_biosignal_sample_t s;
    fovet_hal_biosignal_read(FOVET_SOURCE_ECG, &s);

    ASSERT(s.source == FOVET_SOURCE_ECG,       "ECG sample.source == FOVET_SOURCE_ECG");
...
{
    fovet_hal_biosignal_reset();
    fovet_hal_biosignal_register(FOVET_SOURCE_IMU, stub_error_read);
    fovet_biosignal_sample_t s;
    ASSERT_INT_EQ(fovet_hal_biosignal_read(FOVET_SOURCE_IMU, &s),
                  -42,
...
static void test_reregister_overwrites(void)
{
    fovet_hal_biosignal_reset();
    fovet_hal_biosignal_register(FOVET_SOURCE_IMU, stub_error_read);
    fovet_hal_biosignal_register(FOVET_SOURCE_IMU, stub_imu_read); /* overwrite */
    fovet_biosignal_sample_t s;
    ASSERT_INT_EQ(fovet_hal_biosignal_read(FOVET_SOURCE_IMU, &s),
                  FOVET_HAL_OK,
...
static void test_reset_clears_all(void)
{
    ...
    fovet_biosignal_sample_t s;
    ASSERT_INT_EQ(fovet_hal_biosignal_read(FOVET_SOURCE_IMU,  &s),
                  FOVET_HAL_ERR_NOREG, "reset: IMU cleared");
    ASSERT_INT_EQ(fovet_hal_biosignal_read(FOVET_SOURCE_HR,   &s),
                  FOVET_HAL_ERR_NOREG, "reset: HR cleared");
    ASSERT_INT_EQ(fovet_hal_biosignal_read(FOVET_SOURCE_TEMP, &s),
                  FOVET_HAL_ERR_NOREG, "reset: TEMP cleared");
    ASSERT_INT_EQ(fovet_hal_biosignal_read(FOVET_SOURCE_ECG,  &s),
                  FOVET_HAL_ERR_NOREG, "reset: ECG cleared");
}
...
static void test_all_sources_independent(void)
{
    ...
    fovet_biosignal_sample_t s;
    fovet_hal_biosignal_read(FOVET_SOURCE_IMU, &s);
    ASSERT(s.source == FOVET_SOURCE_IMU,  "independent sources: IMU reads correct source");

    fovet_hal_biosignal_read(FOVET_SOURCE_ECG, &s);
    ASSERT(s.source == FOVET_SOURCE_ECG,  "independent sources: ECG reads correct source");
}

static void test_sample_struct_size(void)
{
    /* Verify layout: 4 + 24 + 4 = 32 bytes on 32-bit platforms */
    ASSERT(sizeof(fovet_biosignal_sample_t) >= 32U,
           "sizeof(fovet_biosignal_sample_t) >= 32");
}
