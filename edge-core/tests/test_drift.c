...
/*
 * Native unit tests for ard_drift — compile with gcc, no hardware needed.
 *
...
#include "../include/ardent/drift.h"
...
{
    ArdentDrift ctx;
    ard_drift_init(&ctx, 0.1f, 0.01f, 3.0f);
...
static void test_alpha_swap_enforced(void)
{
    ...
    ard_drift_init(&ctx, 0.01f, 0.1f, 3.0f);
...
{
    ArdentDrift ctx;
    ard_drift_init(&ctx, 0.1f, 0.01f, 0.001f); /* very tight threshold */

    bool result = ard_drift_update(&ctx, 1000.0f);
...
{
    ArdentDrift ctx;
    ard_drift_init(&ctx, 0.1f, 0.01f, 1.0f);

    /* Constant signal — both EMAs converge to same value, gap stays near 0 */
    int alert_count = 0;
    for (int i = 0; i < 200; i++) {
        if (ard_drift_update(&ctx, 10.0f)) {
            alert_count++;
        }
    }
    ASSERT(alert_count == 0, "constant signal: no drift alert");
    ASSERT_FLOAT_EQ(ard_drift_get_magnitude(&ctx), 0.0f, 1e-3f, "constant signal: magnitude ~= 0");
}
...
static void test_step_drift_detected(void)
{
    ...
    for (int i = 0; i < 100; i++) {
        ard_drift_update(&ctx, 0.0f);
    }
    ASSERT(ard_drift_get_magnitude(&ctx) < 1.0f, "step: no alert before step");

    /* Phase 2: signal jumps to 20.0 — fast EMA reacts, slow EMA lags */
    int alert_count = 0;
    for (int i = 0; i < 100; i++) {
        if (ard_drift_update(&ctx, 20.0f)) {
            alert_count++;
        }
    }
...
static void test_slow_ramp_detected(void)
{
    ...
    ard_drift_init(&ctx, 0.1f, 0.01f, 0.5f);
    ...
    for (int i = 0; i < 200; i++) {
        ...
        if (ard_drift_update(&ctx, sample)) {
            alert_count++;
        }
    }
...
static void test_zscore_misses_drift_but_ewma_catches(void)
{
     ...
     */
    ArdentDrift drift_ctx;
    ArdentZScore zscore_ctx;

    ard_drift_init(&drift_ctx,   0.1f, 0.01f, 0.5f);
    ard_zscore_init(&zscore_ctx, 3.0f, 10U);

    /* Seed both detectors */
    for (int i = 0; i < 50; i++) {
        ard_drift_update(&drift_ctx, 0.0f);
        ard_zscore_update(&zscore_ctx, 0.0f);
    }
    ...
    for (int i = 0; i < 300; i++) {
        ...
        if (ard_zscore_update(&zscore_ctx, sample)) zscore_alerts++;
        if (ard_drift_update(&drift_ctx,   sample)) drift_alerts++;
    }
...
{
    ArdentDrift ctx;
    ard_drift_init(&ctx, 0.1f, 0.01f, 5.0f);

    for (int i = 0; i < 10; i++) {
        ard_drift_update(&ctx, (float)i);
    }
    ard_drift_reset(&ctx);
...
{
    ArdentDrift ctx;
    ard_drift_init(&ctx, 0.1f, 0.01f, 99.0f);

    ard_drift_update(&ctx, 0.0f);   /* seed */
    for (int i = 0; i < 100; i++) {
        ard_drift_update(&ctx, 10.0f);
    }

    float mag = ard_drift_get_magnitude(&ctx);
    ASSERT(mag > 0.0f, "magnitude > 0 after step change");
    ASSERT_FLOAT_EQ(mag, fabsf(ard_drift_get_fast(&ctx) - ard_drift_get_slow(&ctx)),
...
int main(void)
{
    printf("=== Ardent Drift Unit Tests ===\n\n");
