...
static void load_bpm_signal(double bpm, uint32_t n)
{
    ...
    {
        double t = (double)i / (double)FOVET_MAX30102_SAMPLE_RATE;
...
/* Fill the window by calling fovet_hal_hr_read N times.
 * Optionally captures the last sample into *out.
 * Returns the return code of the last call. */
static int fill_window(uint32_t n, fovet_biosignal_sample_t *out)
{
    fovet_biosignal_sample_t sample;
    memset(&sample, 0, sizeof(sample));
    int rc = FOVET_HR_ERR_NODATA;
    for (uint32_t i = 0; i < n; i++)
    {
        prime_fifo_one();
        rc = fovet_hal_hr_read(&sample);
    }
...
static void setup(void)
{
    ...
    s_regs[0xFFU] = FOVET_MAX30102_PART_ID;   /* correct PART_ID */
    ...
    fovet_max30102_set_i2c(mock_write, mock_read);
}
...
{
    setup();
    int rc = fovet_max30102_init();
    ASSERT_EQ(rc, FOVET_HAL_OK);
}
...
static void test_init_registers_source_hr_in_biosignal_hal(void)
{
    setup();
    fovet_max30102_init();
    load_flat_signal(FOVET_MAX30102_WINDOW_SIZE);

    fovet_biosignal_sample_t s;
    /* Fill the window via the biosignal HAL — should NOT return FOVET_HAL_ERR_NOREG (-3) */
    int last_rc = FOVET_HR_ERR_NODATA;
    for (uint32_t i = 0; i < FOVET_MAX30102_WINDOW_SIZE; i++)
    {
        prime_fifo_one();
        last_rc = fovet_hal_biosignal_read(FOVET_SOURCE_HR, &s);
    }
    /* NODATA (-4) on warm-up or OK (0) when full — never NOREG (-3) */
    ASSERT(last_rc != FOVET_HAL_ERR_NOREG);
}
...
static void test_read_returns_nodata_when_fifo_empty(void)
{
    ...
    fovet_biosignal_sample_t s;
    int rc = fovet_hal_hr_read(&s);
    ASSERT_EQ(rc, FOVET_HR_ERR_NODATA);
}
...
static void test_read_returns_nodata_during_warmup(void)
{
    ...
    /* First WINDOW_SIZE-1 calls must all return FOVET_HR_ERR_NODATA */
    for (uint32_t i = 0; i < FOVET_MAX30102_WINDOW_SIZE - 1U; i++)
    {
        prime_fifo_one();
        last_rc = fovet_hal_hr_read(&s);
        if (last_rc != FOVET_HR_ERR_NODATA) break;
    }
    ASSERT_EQ(last_rc, FOVET_HR_ERR_NODATA);
}
...
static void test_read_returns_ok_on_100th_sample(void)
{
    ...
    fovet_max30102_init();
    load_flat_signal(FOVET_MAX30102_WINDOW_SIZE);
    int rc = fill_window(FOVET_MAX30102_WINDOW_SIZE, NULL);
    ASSERT_EQ(rc, FOVET_HAL_OK);
}
...
static void test_bpm_zero_before_window_full(void)
{
    ...
    ASSERT_NEAR(fovet_max30102_get_spo2(), 0.0f, 1.0f);
}
...
static void test_bpm_detected_at_60bpm(void)
{
    setup();
    fovet_max30102_init();
    load_bpm_signal(60.0, FOVET_MAX30102_WINDOW_SIZE);

    fovet_biosignal_sample_t s;
    fill_window(FOVET_MAX30102_WINDOW_SIZE, &s);
...
static void test_bpm_detected_at_80bpm(void)
{
    setup();
    fovet_max30102_init();
    load_bpm_signal(80.0, FOVET_MAX30102_WINDOW_SIZE);
...
static void test_bpm_in_physiological_range(void)
{
    setup();
    fovet_max30102_init();
    load_bpm_signal(72.0, FOVET_MAX30102_WINDOW_SIZE);
...
static void test_spo2_normal_range_for_standard_signal(void)
{
    ...
    fovet_max30102_init();
    load_bpm_signal(60.0, FOVET_MAX30102_WINDOW_SIZE);
    fill_window(FOVET_MAX30102_WINDOW_SIZE, NULL);

    float spo2 = fovet_max30102_get_spo2();
    /* Expect 95–100% */
...
static void test_spo2_clamped_to_100_on_high_value(void)
{
    ...
    s_fifo_total  = FOVET_MAX30102_WINDOW_SIZE;
    s_fifo_served = 0;
    fill_window(FOVET_MAX30102_WINDOW_SIZE, NULL);

    ASSERT(fovet_max30102_get_spo2() <= 100.0f);
}
...
static void test_spo2_clamped_to_0_on_low_value(void)
{
    ...
    ASSERT(fovet_max30102_get_spo2() >= 0.0f);
}
...
static void test_biosignal_read_delegates_to_hr_driver(void)
{
    ...
    {
        prime_fifo_one();
        rc = fovet_hal_biosignal_read(FOVET_SOURCE_HR, &s);
    }
    ASSERT_EQ(rc, FOVET_HAL_OK);
}
...
static void test_biosignal_sample_source_is_hr(void)
{
    ...
    ASSERT_EQ(s.source, FOVET_SOURCE_HR);
}
...
static void test_reset_clears_window_and_bpm(void)
{
    ...
    fovet_max30102_reset();
    fovet_max30102_set_i2c(mock_write, mock_read);
    s_regs[0xFFU] = FOVET_MAX30102_PART_ID;
    prime_fifo_one();
    load_flat_signal(1);
    int rc = fovet_hal_hr_read(&s);

    ASSERT_GT(bpm_before, 0.0f);          /* had a BPM before reset     */
    ASSERT_EQ(rc, FOVET_HR_ERR_NODATA);   /* back to warm-up after reset */
}
